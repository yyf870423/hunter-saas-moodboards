...
Hunter SaaS 的专业猎头视觉与交互方向研究，共包含 20 套结构明显不同的 Moodboard。每套方案都有一个独立的工作模型，并提供两个页面：

- **Dashboard：** 用真实 Hunter 业务展示页面布局、信息层级和核心工作方式。
- **组件与交互：** 展示完整按钮、表单、数据、业务资产、思维导图、状态、浮层和四组专属业务动效。
...
Playwright 覆盖 20 套方案的 Dashboard 和组件与交互页，并分别验证桌面、iPad 和 iPhone，共 120 个响应式页面组合；其余用例验证首页、方案独立结构、完整组件、思维导图、动效触发和废弃路由。

## 工程边界

- 使用薄 HTML 入口、共享设计 token、按家族分发的 Dashboard 布局和按方案变化的完整组件系统。
- 允许复用无障碍、状态和数据逻辑，但 20 套方案的布局、组件材质、思维导图和业务动效必须保持可辨识差异。
